...
This is the `sf-marketing-solutions` repository containing Snowflake MAM (Marketing, AdTech & MarTech) industry solution accelerators. Each solution lives in `solutions/<name>/` with a standard structure (manifest.json, scripts/, streamlit/).
...
|----------|---------|---------|
| Cortex Code | `$sf-solutions:<name>` | `$sf-solutions:<example-solution>` |
...
| ssp-impression-analytics | Advertising & MarTech | SF_SOLUTIONS | SSP_IMPRESSION_ANALYTICS | Semantic View, Cortex Agent, Identity Resolution |
